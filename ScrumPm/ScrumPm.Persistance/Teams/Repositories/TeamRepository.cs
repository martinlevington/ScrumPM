...
namespace ScrumPm.Persistence.Teams.Repositories
{
    public class TeamRepository : Repository<Team, int, TeamEf>, ITeamRepository
    {
        ...
        public TeamRepository(UnitOfWorkEf unitOfWork, IMapper mapper, ITeamAdapterFactory teamAdapterFactory)
        {
        ...
        {
            var teams = _unitOfWork.GetContext().Teams.Include(t => t.ProductOwner).Select(x => x).ToList();
            ...
            ISpecification<Team> specification)
        {
            ...
            var teams = _unitOfWork.GetContext().Teams.Include(t => t.ProductOwner).Where(expression).ToList();
        ...
        {
            var team = _unitOfWork.GetContext().Teams.Include(t => t.ProductOwner)
